...
def get_classifier(clf_name, train_set):    
    ...
    if clf_name in ["linear", "logistic"]:
        batch = train_set[0]
        # print(batch[0].shape)
        model = Mlp_model(input_size=batch[0].shape[0], hidden_sizes=[], n_classes=2, bias=False)
    ...
    if clf_name == "mlp_dropout":
        model = Mlp(n_classes=10, dropout=True)

    elif clf_name == "resnet34":
        model = ResNet([3, 4, 6, 3], num_classes=10)

    elif clf_name == "resnet34_100":
        model = ResNet([3, 4, 6, 3], num_classes=100)

    elif clf_name == "resnet34_200":
        model = ResNet([3, 4, 6, 3], num_classes=200)

    elif clf_name == "resnet34_nobn":
        model = ResNet([3, 4, 6, 3], num_classes=10)
        model.apply(deactivate_batchnorm)

    elif clf_name == "resnet34_100_nobn":
        model = ResNet([3, 4, 6, 3], num_classes=100)
        model.apply(deactivate_batchnorm)

    elif clf_name == "resnet18":
        model = models.resnet18()



    elif clf_name == "resnet50":
        model = models.resnet50()
    
    elif clf_name == "resnet101":
        model = models.resnet101()
    
    elif clf_name == "resnet152":
        model = models.resnet152()
    
    elif clf_name == "mxresnet50":
        model = mxresnet.mxresnet50(sa=1)

    elif clf_name == "wide_resnet101":
        model = models.wide_resnet101_2()

    elif clf_name == "densenet121":
        model = DenseNet121(num_classes=10)

    elif clf_name == "densenet121_100":
        model = DenseNet121(num_classes=100)

    elif clf_name == "densenet121_nobn":
        model = DenseNet121(num_classes=10)
        model.apply(deactivate_batchnorm)

    elif clf_name == "densenet121_100_nobn":
        model = DenseNet121(num_classes=100)
        model.apply(deactivate_batchnorm)

    elif clf_name == "matrix_fac_1":
        model = LinearNetwork(6, [1], 10, bias=False)
        
    elif clf_name == "matrix_fac_4":
        model = LinearNetwork(6, [4], 10, bias=False)
    
    elif clf_name == "matrix_fac_10":
        model = LinearNetwork(6, [10], 10, bias=False)

    elif clf_name == "linear_fac":
